...
must_haves:
  ...
  key_links:
    ...
    - from: "apps/web/src/app/api/cron/calendar-renewal/route.ts"
      to: "apps/web/src/lib/calendar/renewal.ts"
      via: "fallbackPollAllUsers() call as reliability fallback"
      pattern: "fallbackPollAllUsers"
---
...
<task type="auto">
  ...
  <action>
    ...
    **apps/web/src/lib/calendar/watcher.ts:**
    ...
    - **Check the user's auto-join preference** before creating td_meetings rows:
      - Query td_users for the user's `calendarAutoJoin` and `calendarKeywords` values
      - If `calendarAutoJoin === "disabled"`: skip all events, log "User {userId} has auto-join disabled"
      - If `calendarAutoJoin === "keywords"`: only process events where `event.summary` matches one of the comma-separated keywords (case-insensitive partial match)
      - If `calendarAutoJoin === "all"` (default): process all events with Meet links
    - For each qualifying event that has `conferenceData.entryPoints` with type "video" and a Google Meet URI:
...
<task type="auto">
  ...
  <action>
    ...
    **apps/web/src/lib/calendar/watcher.ts (UPDATE):**
    Ensure `detectAndDispatchMeetings` checks the user's auto-join preference (calendarAutoJoin, calendarKeywords from td_users) BEFORE creating td_meetings rows, as described in Task 1's action. This is the same file from Task 1 — if the preference check was already added in Task 1, verify it is present. If not, add it now:
    - Query td_users for the user's `calendarAutoJoin` value
    - If "disabled", skip event processing entirely
    - If "keywords", filter events by keyword match on event.summary
    - If "all" (default), process all events with Meet links
    ...
    **apps/web/src/app/api/cron/calendar-renewal/route.ts:**
    ...
    3. Logs completion: `console.log("[cron/calendar-renewal] Renewal complete: renewed=${renewed}, polled=${polled}")`
    4. Returns JSON with counts: `{ renewed: N, polled: M }`
    5. Protection: check `Authorization: Bearer ${process.env.CRON_SECRET}` header. Reject if missing/wrong.
    ...
    **apps/web/src/app/(dashboard)/settings/page.tsx:**
    Create a settings page with **server-side admin role enforcement**:
    - At the TOP of the page component, call `auth()` to get the session
    - Query td_users for the current user's role
    - **If `session.user.role !== "admin"`, redirect to `/meetings` using `redirect()` from `next/navigation`** — this is the server-side role gate (AUTH-02 requirement)
    ...
    - For CAL-04 (meeting auto-join configuration): Add a simple form to configure which meetings to auto-join. For MVP, offer three options stored in td_users:
      ...
      3. "Disabled" (opt out)
    - Store the preference. The `detectAndDispatchMeetings` function checks this preference before creating td_meetings rows (wired in Task 1 and verified in watcher.ts above).

    **Schema update for meeting preferences:**
    Add to td_users:
    ```typescript
  ...
  <verify>
    ...
    2. Run the cron endpoint and verify log output: `CRON_SECRET=test-cron-secret docker compose logs web --tail 30 2>&1 | grep "cron/calendar-renewal"` — shows renewal log entry after running the cron endpoint
    3. `curl http://localhost:3000/api/cron/calendar-renewal` (no auth) — returns 401
    4. TypeScript compiles: `cd apps/web && npx tsc --noEmit`
    5. Settings page loads at /settings (when authenticated as admin)
    6. Settings page redirects non-admin users: log in as a member-role user and visit /settings — should redirect to /meetings
    7. Schema pushed: `docker compose exec postgres psql -U meetrec -c "\d td_users"` shows calendar_auto_join column
    8. Renewal logic handles empty channel list gracefully (returns 0, no errors)
  </verify>
  <done>
    Webhook renewal cron renews expiring channels within 48h, confirmed by a log entry after each renewal run. Fallback hourly poll catches missed events. Calendar renewal endpoint is protected by cron secret. Settings page is admin-only with server-side role enforcement (non-admin users redirected to /meetings). Settings page lets admin connect calendars and configure auto-join preferences. The detectAndDispatchMeetings function in watcher.ts checks auto-join preferences before creating td_meetings rows. CAL-03 (renewal), CAL-04 (configuration), and AUTH-02 (admin-only settings access) requirements are satisfied.
  </done>
</task>
...
4. Renewal cron finds and renews expiring channels, with log output confirming each run
5. Fallback poll catches events when webhook is down
6. Meeting auto-join preferences are configurable per user and enforced during event detection
7. Token refresh works when access token expires
8. Settings page is admin-only — non-admin users are redirected server-side
</verification>
...
- CAL-03 satisfied: Background job renews Calendar webhook channels before 7-day expiry, confirmed by log entry
- CAL-04 satisfied: User can configure which meetings to auto-join
- AUTH-02 satisfied: Settings page enforces admin-only access via server-side role check
