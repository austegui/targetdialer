...
must_haves:
  truths:
    ...
    - "At least one transcript segment has a non-null speaker value, confirming diarization is active"
...
<task type="auto">
  <name>Task 1: VexaClient and bot dispatch endpoint</name>
  <files>
    apps/api/src/vexa_client.py
    apps/api/src/main.py
    apps/web/src/lib/calendar/watcher.ts
  ...
  <verify>
    ...
    3. Python files have no syntax errors: `cd apps/api && python -m py_compile src/vexa_client.py`
    4. TypeScript compiles: `cd apps/web && npx tsc --noEmit`
  </verify>
  <done>
    VexaClient wraps all Vexa API calls. Bot dispatch endpoint receives meeting IDs from Calendar Watcher and starts Vexa bots. td_meetings.bot_status updates to "joining" on dispatch. Calendar Watcher calls the dispatch endpoint after creating td_meetings rows.
  </done>
</task>

<task type="auto">
  <name>Task 2: Webhook receiver, transcript persistence, and bot_status synchronization</name>
  <files>
    apps/api/src/webhook/vexa.py
    apps/api/src/jobs/transcript.py
    apps/api/src/main.py
  ...
  <action>
    **apps/api/src/webhook/vexa.py:**
    ...
    - Parse the incoming JSON payload. **IMPORTANT:** Log the raw payload on first receipt — the exact schema is not 100% confirmed. Use defensive parsing:
      ...
      - Also look for a `status` field — Vexa may send status-update webhooks (e.g., bot transitioned to "active") in addition to transcript completion webhooks
    ...
    - **For status-update payloads:** If the payload contains a status field indicating the bot is now active, UPDATE td_meetings SET `bot_status = 'active'` WHERE `vexa_meeting_id = native_meeting_id`. Log: "Bot status updated to active for meeting {meeting_id}"
    ...
    **apps/api/src/main.py (UPDATE):**
    - Include the vexa webhook router in the FastAPI app
    - In the dispatch endpoint, after successful bot dispatch response from Vexa, update td_meetings `bot_status = 'joining'`
    - Add a periodic status sync mechanism: in the health check (Task 3), query Vexa for current bot status via `vexa_client.get_bot_status()` and update td_meetings.bot_status accordingly — this ensures td_meetings reflects the real Vexa state even if a status webhook was missed
  </action>
  <verify>
    1. Webhook endpoint exists: `curl -X POST http://localhost:8000/webhook/vexa -H "Content-Type: application/json" -d '{"native_meeting_id":"test","platform":"google_meet","segments":[]}'` — returns `{"status": "accepted"}`
    2. Webhook with status update works: `curl -X POST http://localhost:8000/webhook/vexa -H "Content-Type: application/json" -d '{"native_meeting_id":"test-status","platform":"google_meet","status":"active"}'` — returns `{"status": "accepted"}` and logs status update
    3. Python files have no syntax errors: `cd apps/api && python -m py_compile src/webhook/vexa.py && python -m py_compile src/jobs/transcript.py`
    4. After sending a mock transcript webhook with segments, check td_meetings.bot_status was updated: `docker compose exec postgres psql -U meetrec -c "SELECT vexa_meeting_id, bot_status FROM td_meetings WHERE vexa_meeting_id = 'test';"` — shows bot_status reflecting the latest update
    5. Bot status transitions are observable: dispatch sets "joining", Vexa status webhook sets "active", transcript completion sets "completed"
  </verify>
  <done>
    Vexa webhook receiver accepts both status-update and transcript-completion payloads. Status updates transition bot_status from "joining" to "active" in td_meetings. Transcript segments are enqueued as ARQ jobs and persisted to td_transcript_segments with idempotent inserts. Bot status transitions (joining -> active -> completed) are reflected in td_meetings, enabling the health check to identify ACTIVE+no-segment bots.
  </done>
</task>

<task type="auto">
  <name>Task 3: Bot health check (deaf bot detection) and ARQ worker setup</name>
  <files>
    apps/api/src/jobs/health_check.py
    apps/api/src/jobs/worker.py
    apps/api/src/main.py
  ...
  <action>
    **apps/api/src/jobs/health_check.py:**
    ...
    3. **Also sync bot status from Vexa:** For all td_meetings where `bot_status = 'joining'` and `bot_joined_at < NOW() - INTERVAL '30 seconds'`, call `vexa_client.get_bot_status(platform, meeting_id)` and update td_meetings.bot_status to match Vexa's reported state. This ensures the "joining" -> "active" transition is captured even if the status webhook was missed.
    4. Return count of bots restarted
  ...
  <verify>
    ...
    6. Health check syncs bot status: insert a test td_meetings row with bot_status='joining' and bot_joined_at=NOW()-5min, run health check, verify bot_status was updated (or attempted to sync with Vexa)
  </verify>
  <done>
    ARQ worker runs as a separate Docker service processing transcript save jobs and periodic health checks. Deaf bot detection (ACTIVE status + no segments for 3 minutes) triggers automatic restart with max 2 retries. Health check also syncs bot_status from Vexa for "joining" bots, ensuring the joining->active transition is reflected in td_meetings even if status webhooks are missed. Bot status transitions are reflected in td_meetings, enabling health check to identify ACTIVE+no-segment bots. Roadmap success criteria #5 met.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    3. Test the webhook receiver with mock transcript data INCLUDING speaker labels:
       ```
       curl -X POST http://localhost:8000/webhook/vexa \
         -H "Content-Type: application/json" \
         -d '{"native_meeting_id":"abc-defg-hij","platform":"google_meet","segments":[{"text":"Hello world","speaker":"Speaker 1","start_time":"0.0","end_time":"2.5","language":"en"},{"text":"Hi there","speaker":"Speaker 2","start_time":"2.6","end_time":"4.0","language":"en"}]}'
       ```
       Expected: `{"status": "accepted"}`
    4. After webhook test, check transcript was saved WITH speaker labels (give worker a few seconds):
       ```
       docker compose exec postgres psql -U meetrec -c "SELECT text, speaker FROM td_transcript_segments WHERE vexa_meeting_id = 'abc-defg-hij';"
       ```
       Expected: Two rows — "Hello world" with speaker "Speaker 1", "Hi there" with speaker "Speaker 2"
    5. **Verify diarization is active — at least one segment has a non-null speaker value:**
       ```
       docker compose exec postgres psql -U meetrec -c "SELECT COUNT(*) as segments_with_speaker FROM td_transcript_segments WHERE speaker IS NOT NULL;"
       ```
       Expected: Count >= 1 (confirms speaker diarization data is being persisted)
    6. Verify health check logic exists and ARQ worker is running:
       ```
       docker compose logs worker --tail 20
       ```
       Expected: Worker startup logs, periodic health check runs
    7. Check the full data flow in the database:
       ```
       docker compose exec postgres psql -U meetrec -c "SELECT vexa_meeting_id, bot_status, segment_count FROM td_meetings;"
       ```
  </how-to-verify>
  <resume-signal>Type "approved" if the dispatch, webhook, transcript storage (with speaker labels), and health check work correctly, or describe any issues. Full end-to-end testing with a real Vexa instance and Google Meet call can be deferred to integration testing.</resume-signal>
</task>
...
2. Bot dispatch creates/updates td_meetings rows with bot_status transitions (requested -> joining -> active -> completed)
3. Vexa webhook receives both status-update and completion payloads
4. Transcript segments are persisted in td_transcript_segments with speaker labels (non-null speaker values)
5. Health check detects deaf bots (ACTIVE + no segments after 3 min) and restarts them
6. Health check syncs bot_status from Vexa for "joining" bots
7. ARQ worker runs as a Docker service processing jobs
8. End-to-end data flow: calendar event -> td_meetings row -> bot dispatch -> transcript webhook -> td_transcript_segments
</verification>

<success_criteria>
- REC-01 satisfied: Vexa bot joins Google Meet (via dispatch from Calendar Watcher)
- REC-02 satisfied: Real-time transcription with timestamps and speaker diarization stored — verified by at least one non-null speaker value in td_transcript_segments
