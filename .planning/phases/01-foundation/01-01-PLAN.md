...
must_haves:
  truths:
    ...
    - "Full-text search is indexed from day one — a test FTS query on td_transcript_segments returns results after first insert"
    - "Both services respond to HTTP health checks confirming they can reach the database"
...
<task type="auto">
  ...
  <action>
    ...
    **After creating all files, run schema push:**
    ...
    6. Verify FTS works: Insert a test row and run a tsvector query to confirm the GIN index is functional:
       ```sql
       INSERT INTO td_transcript_segments (id, vexa_meeting_id, platform, text) VALUES (gen_random_uuid(), 'test-fts', 'test', 'Hello world testing full text search');
       SELECT id, text FROM td_transcript_segments WHERE to_tsvector('english', text) @@ to_tsquery('english', 'testing');
       DELETE FROM td_transcript_segments WHERE vexa_meeting_id = 'test-fts';
       ```
       The SELECT must return the inserted row. Delete the test row after confirming.
  </action>
  <verify>
    ...
    5. FTS query returns results: `docker compose exec postgres psql -U meetrec -c "INSERT INTO td_transcript_segments (id, vexa_meeting_id, platform, text) VALUES (gen_random_uuid(), 'test-verify', 'test', 'verify full text search'); SELECT count(*) FROM td_transcript_segments WHERE to_tsvector('english', text) @@ to_tsquery('english', 'verify'); DELETE FROM td_transcript_segments WHERE vexa_meeting_id = 'test-verify';"` — count is 1
    6. `docker compose up -d` — all services start (web on 3000, api on 8000)
    7. `curl http://localhost:3000` — returns HTML (Next.js placeholder page)
    8. `curl http://localhost:8000/health` — returns `{"status": "ok", "service": "meetrec-api"}`
  </verify>
  <done>
    All four TD extension tables created with correct columns and types. GIN index on transcript text confirmed and FTS query returns results on first insert. Both Next.js and FastAPI services running and responding. Docker Compose stack fully operational with PostgreSQL and Redis.
  </done>
</task>
...
4. FTS query against td_transcript_segments returns results after a test insert
5. `curl http://localhost:3000` returns Next.js page
6. `curl http://localhost:8000/health` returns 200 with OK status
7. Both services can connect to PostgreSQL (web via drizzle-kit push, api via health check)
</verification>
...
- GIN full-text search index exists and a test FTS query returns results
